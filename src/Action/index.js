...
export const GET_DATA_BY_CONDITION_JSON_FILE =
  "GET_DATA_BY_CONDITION_JSON_FILE";
...
export const SET_CREATE_RELATIONSHIP = "SET_CREATE_RELATIONSHIP";
...
export function setCreateRelationShip(relationship) {
  return async (dispatch) => {
    dispatch({ type: "SET_CREATE_RELATIONSHIP", data: relationship });
  };
}
...
export function createRelationship(relationShip) {
  return async (dispatch) => {
    try {
      const res = await axios_instance.post(
        "/queryNodeCreateRelation",
        relationShip
      );
      if (res.data.code === 200) {
        dispatch({ type: "CREATE_RELATIONSHIP", data: res.message });
      } else {
        dispatch({
...
export function deleteRelationship(relationship) {
  return async (dispatch) => {
    try {
      const res = await axios_instance.post("/deleteRelation", relationship);
      if (res.data.code === 200) {
        dispatch({ type: "DELETE_RELATIONSHIP", data: res.message });
      } else {
        dispatch({
